...
def translate_worker_gemini(chunk, attempt_limit=5):
    """Translates a batch of footnotes with recursive splitting and manual JSON repair."""
    ...
    for attempt in range(attempt_limit):
        try:
            ...
            if is_truncated:
                print(f"    [Repair] JSON truncated. Attempting recovery...")
            ...
            if (is_truncated or len(parsed) < len(chunk)) and len(chunk) > 1:
                print(f"    [Fallback] Incomplete batch ({len(parsed)}/{len(chunk)}). Splitting...")
            ...
            err_msg = str(e)
            if "RESOURCE_EXHAUSTED" in err_msg or "ConnectError" in err_msg:
                wait_time = 45 * (attempt + 1)
                print(f"    [Limit/Network] Waiting {wait_time}s...")
                time.sleep(wait_time)
                continue # Retry same chunk

            if len(chunk) > 1:
                print(f"    [Error] Splitting batch due to: {err_msg[:50]}")
                items = list(chunk.items())
                mid = len(items) // 2
                r1 = translate_worker_gemini(dict(items[:mid]))
                r2 = translate_worker_gemini(dict(items[mid:]))
            ...
            print(f"    [Fail] Attempt {attempt+1}: {err_msg[:100]}")
            time.sleep(5)
                
    if len(chunk) == 1:
        fn_id = list(chunk.keys())[0]
        log_failure(fn_id, "Max retries reached.")
    
    return None

def get_chapter_name(chunk):
    """Extracts a readable chapter name from the parent_path of the first item."""
    try:
        first_val = list(chunk.values())[0]
        path = first_val.get('parent_path', '')
        parts = path.split('.')
        # root.text.Part X Ch Y.0 -> Part X Ch Y
        if len(parts) > 3: return ".".join(parts[2:-1])
        return parts[2] if len(parts) > 2 else "Unknown"
    except:
        return "Unknown"

def run_footnote_rehab():
    print(f"\n--- Restored Gemini Footnote Pipeline (Model: {MODEL_NAME}) ---")
